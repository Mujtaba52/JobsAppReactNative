...
export const SearchJobs =
  ...
  async (dispatch) => {
    try {
      ...
      const {
        data: { data },
      } = await api.fetchSearchJob(search);
      if (data.length > 0) {
...
export const RecommendedJobs = (user, tag) => async (dispatch) => {
  try {
    ...
    const {
      data: { data },
    } = await api.fetchRecommendedJobs(user, tag);
    if (data.length > 0) {
      dispatch({
        type: GET_RECOMMENDED_JOBS,
        payload: { recommendedJobs: data },
      });
      dispatch({ type: SUCCESS });
...
export const JobByID = (user, id) => async (dispatch) => {
  try {
    ...
    const {
      data: { data },
    } = await api.fetchJobByID(user, id);
...
export const CompanyJobs = (user, id) => async (dispatch) => {
  try {
    ...
    const {
      data: { data },
    } = await api.fetchJobByCompany(user, id);
    if (data.length > 0) {
...
export const ProviderJobs = (id) => async (dispatch) => {
  try {
    ...
    const {
      data: { data },
    } = await api.fetchJobsByProvider(id);
    if (data.length > 0) {
...
export const FeaturedProviderJobs = (id) => async (dispatch) => {
  try {
    ...
    const {
      data: { data },
    } = await api.fetchJobsByProviderFeatured(id);
    if (data.length > 0) {
      dispatch({
        type: GET_JOBS_BY_PROVIDER_FEATURED,
        payload: { providerJobsFeatured: data },
      });
      dispatch({ type: SUCCESS });
